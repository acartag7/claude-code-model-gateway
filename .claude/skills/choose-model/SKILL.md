...
4. Default to safe context mode. Select experimental full-context mode only
   when the user explicitly accepts its unsupported Claude Code ceiling shim.
5. Name an explicit fallback. Never silently substitute it.
6. Return the exact Claude Code model value and effort level.
